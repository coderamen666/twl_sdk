...
static void OnVBlank(void)
{
    ...
    if ((p->draw_flag & DEMO_DRAW_FLIPPING) != 0)
    {
        ...
        /* Transfer the surface that was being drawn up until just now */
        MI_CpuCopyFast(tmp_surface, dst_vram, sizeof(tmp_surface));
        /* Clear if an initial color has been set */
...
void InitDraw(void)
{
    ...
    if (!p->draw_flag)
    {
        ...
        /* Configuration of shared render variables */
        ...
        /* VRAM-C direct display for dump displays and so on in half-width characters */
