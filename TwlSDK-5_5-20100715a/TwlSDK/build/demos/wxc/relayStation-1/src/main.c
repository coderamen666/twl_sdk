...
/*---------------------------------------------------------------------------*
    ...
    Child mode becomes 100%, and it operates identically to a relay station
 *---------------------------------------------------------------------------*/
...
BOOL passby_ggid[MENU_MAX];     // GGID that does chance encounter
...
void NitroMain(void)
{
    ...
    while(1)
    {
        ...
        if(menu_flg == 9)
        {
            ...
            // Continues to act as a relay station until the start button is pressed
