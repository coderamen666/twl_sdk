...
/*
  ...
  Although this is not in game format, this sample program actively implements the features that are necessary (that seem necessary) for an actual game application.
  
  Specific features implemented include those listed below.
  
  1) Lobby screen
  2) Parent selection screen
  3) Measures against process slow-down (synchronous)
  4) Error display and return
  
  
  For normal controls, the +Control Pad makes a menu selection, the A Button confirms, and the B Button cancels. For additional debug controls, the L Button and R Button respectively increase and decrease the load count.
...
static void scanCallback(WMBssDesc *bssdesc)
{
    ...
    // Check whether the userGameInfo is an expected value
    if (bssdesc->gameInfoLength != 0 && bssdesc->gameInfo.userGameInfoLength == sizeof(MyGameInfo))
    {
        ...
        // Do not trust data obtained via communications. Always check that the values are appropriate.
    ...
    // Investigate the detected parents
    ...
    {
        // Overwrite because it is already registered in the menu
        setItemToWindow(&sSelectParentWindow, buf, i);
    }
    else
    {
        // The information is stored and added to the menu because a new parent was detected
...
static void ModeSelectRole(void)
{
    ...
    if (sRoleMenuWindow.state == WIN_STATE_CLOSED)
    {
        ...
        case 0:
            ...
            if (sForcedChannel == 0)
            {
                // Get the best channel based on the usage rate and makes a connection
            ...
            else
            {
                ...
                // Update userGameInfo while in the entry reception state
                updateGameInfo(TRUE);
...
static void ModeSelectParent(void)
{
    ...
    if ((sSelectParentWindow.state == WIN_STATE_CLOSED))
    {
        ...
        if (WH_GetSystemState() == WH_SYSSTATE_IDLE)
        {
            ...
            // When connecting to a parent, set the local host's nickname in the SSID user region and send a notification
...
static void VBlankIntr(void)
{
    /*
       ...
       The SDK prepares a VCount of 240 for child devices and 260 for parent devices with default MP communications, so the fewest problems are presented by setting a value immediately after the VBlank starts.
       (Although not implemented when this document was written, there are plans to allow the VCount for the start of MP communications to be set during initialization.)
...
{
    // GUIDELINE
    // Display the radio reception strength icon
    setupPseudo2DCamera();
    ...
    for (sFrame = 0; TRUE; sFrame++)
    {
        ...
        case WH_SYSSTATE_CONNECT_FAIL:
            ...
            if (sSysMode == SYSMODE_LOBBYWAIT && retry < MAX_RETRY)
            {
                ...
                sSysModeStep = 1; // So that window does not open...
                retry++;
                break;
            }
        ...
        if (!sNeedWait && ((sSysMode == SYSMODE_PARENT) || (sSysMode == SYSMODE_CHILD)))
        {
            ...
            // Update the balls' state using data that could be shared.
            //   Note that if this is run in a frame during which WH_StepDS failed, synchronization between the parent and child game state will be lost.
            //   
...
#endif
{
    ...
    // Set the callback specified in WM_SetIndCallback, which is called within WH
