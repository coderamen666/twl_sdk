...
#define TBL849f_ELEM_NUM				0x20    // Number of elements in the 849ftbl
        ...
        // Value for determining SJIS codes
...
//======================================================
// Conversion from SJIS-BE to UTF16-LE
//======================================================
void ExSJIS_BEtoUTF16_LE(u8 *sjisp, u16 *unip, u16 length)
{
    ...
    while ((*sjisp) && (length-- > 0))
    {
        ...
        else if ((code >= 0x8340) && (code <= 0x8396))  // É@ Å` Éñ  * 0x837f is skipped
        {
        ...
        else if ((code >= 0x8470) && (code <= 0x8491))  // Ñp Å` Ñë * 0x847f is skipped
        {
...
//======================================================
// Conversion from UTF16-LE to SJIS-BE
//======================================================
void ExUTF16_LEtoSJIS_BE(u8 *sjisp, u16 *unip, u16 length)
{
    ...
    while ((*unip) && (length-- > 0))
    {
        ...
        else if ((code >= 0x3000) && (code <= 0x30fe))
        {
            ...
            else if ((code >= 0x30a1) && (code <= 0x30f6))      // É@ Å` Éñ  * 0x837f is skipped
            {
        ...
        else if ((code >= 0x0430) && (code <= 0x044f))  // Ñp Å` Ñë * 0x847f is skipped
        {
        ...
        {                              // The aforementioned codes are located in SJIS 0x815c to 0x81fc
            sjis_le =
...
// Table lookup from Unicode -> SJIS
static u16 SearchUnicodeTable(u16 unicode, SjisUtf16Pare * tblp, int elem_num)
{
    elem_num++;
    while (elem_num--)
