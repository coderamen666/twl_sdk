...
#
# This outputs the header file(s) filename1.h (filename2.h) from the CSV file for which macro definitions are listed.
# 
#
# See (SDKRoot)/docs/private/how-to-make-headers.txt for CSV file format.
# 
#
...
sub cull_register_offset_def {
    ...
    while ( 1 ) {
        
        {
            ...
            # Keep this variable definition because it is used later
        ...
        #If reg_type and offset are identical, merge condition
        foreach $k (@def_array) {
            if ( $k->{'condition'} eq "" ) {
...
# If -nodup is specified, duplicate macro names are not allowed
if ($nodup == 1) {
  $duplicate_ok = 0;
} else {
  $duplicate_ok = 1;
