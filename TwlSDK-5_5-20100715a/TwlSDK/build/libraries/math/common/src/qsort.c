...
/*---------------------------------------------------------------------------*
  ...
  Description:  Performs a quicksort without using recursion.
                The buffer area used for sort must be passed, and the required buffer size can be obtained with MATH_QSORT_STACK_SIZE( num ).
...
#define width_      r8
    ...
    mov     comp_, comp                     //  // r0-r3 and r12 are scratch registers, so they are saved before calling the comparison function
    mov     width_, width
    
    cmp     stack, #0
    bne     @00
    clz     tmp, num                        //  // Allocate from the stack when the work buffer is NULL.
    rsb     tmp, tmp, #32
...
@01:
    ...
    ble     @01
                                            //              swap( left, right, width ); /* swap begins here */
    mov     r0, width_
    tst     r0, #3
    beq     @012
@011:
...
@012:
    ...
    b       @01
                                            ...
                                            //          continue;
@02:                                        //      }
                                            //      // Use the ILog2(width) value initially computed to select a reference value at the middle position
                                            //      somewhere = (((right - left) >> ILog2(width)) * width / 2) + left;
    ldr     tmp2, [sp]
    sub     tmp, right, left
    mov     tmp, tmp, lsr tmp2
    mla     tmp, tmp, width_, left
                                            //      swap( left, left + width * 2, width ); /* swap begins here */
    mov     r3, left
    mov     r0, width_
    mov     r2, tmp
    tst     r0, #3
...
@04:
    ...
    bge     @05
                                            ...
                                            //             swap( somewhere_l, somewhere_r, width ) /* swap begins here */;
    mov     r2, somewhere_l
    mov     r3, somewhere_r
    mov     r0, width_
    tst     r0, #3
...
@05:
                                            //      swap( left, somewhere_r, width ); /* swap begins here */
    mov     r2, left
    mov     r3, somewhere_r
    mov     r0, width_
    tst     r0, #3
...
@053:
    ...
    sub     tmp2, right, somewhere_r        //      /* Sort from a small range in order to use the stack as little as possible */
