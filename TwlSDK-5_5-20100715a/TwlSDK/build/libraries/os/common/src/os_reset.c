...
/*---------------------------------------------------------------------------*
  ...
  Description:  Initializes reset system.
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Starts reset sequence.
...
void OS_ResetSystem(u32 parameter)
{
    ...
    *(u32 *)HW_RESET_PARAMETER_BUF = parameter;
    ...
    //---- Get original exception handler and put into temp area to set back later
...
// Bundle all of the SVC processing used at the final stage of a software reset, and back it up to ITCM
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Verifies 4 static segments before rebooting.
...
static void OSi_VerifyStaticSegments(void)
{
    ...
    // If any segment is invalid, enter the infinite loop
    while (result == FALSE)
    {
    }
}
...
@1:
    ...
    //---- Clear some memory
    mov         r0, #0
    ldr         r3, =HW_COMPONENT_PARAM         // save exception vector
    ldr         r4, [r3]
    ...
    ldr         r3, =HW_EXCP_VECTOR_MAIN        // Restore exception vector
    str         r4, [r3]
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Reloads ROM data from card.
...
@shakeHand01_end:
    ...
    // If necessary, reload the ROM header. (Only when running in NTR mode.)
    cmp     r4, #0
    movne   r0, #0
...
@loadLtdStatic:
    ...
    /* Load the extension binary for the ARM9 */
    ...
    cmp     r2, #0
    blgt    OSi_ReadCardRom32
    /* Load the extension binary for the ARM7 */
    ldr     r12, =HW_TWL_ROM_HEADER_BUF
    ldr     r0, [r12, #0x1d0]
    ldr     r1, [r12, #0x1d8]
    ldr     r2, [r12, #0x1dc]
...
//  mov     r0, #0
    ...
    beq     @shakeHand02_end
    /* Synchronize with ARM7 */
    /* It is necessary to recover the beginning portion of the extension binary that was retracted */
    mov     r3, #2
    ldr     r0, =HW_BOOT_SYNC_PHASE
    strh    r3, [r0]
    ldr     r0, =HW_BOOT_SHAKEHAND_9
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Reads card data by CPU.
                (subroutine of OSi_ReloadRomData)

  Arguments:    src: Source address, needs to be 4-byte aligned
                dest: Destination address, needs to be 4-byte aligned
    ...
    for (src = (u32)(src + pos); pos < len; src += 512)
    {
        ...
        *reg_CARD_CNT = ctrl_start;
        for (;;)
        {
            ...
            //---- Note: Need to read whole one page data
...
@restoreShelteredBinary:
    ldr     r12, =HW_TWL_ROM_HEADER_BUF
    /* Recover retracted data from the ARM7 extended binary */
    ldr     r0, [r12, #0x1dc]
    cmp     r0, #0
    beq     @restoreNext
    ldr     r2, [r12, #0x1d8]   // r2 = static module RAM address for ARM7
...
@restoreNext:
    /* Recover retracted data of the extension binary for ARM9 */
    ldr     r0, [r12, #0x1cc]
    cmp     r0, #0
    beq     @shakeHand02
    ldr     r2, [r12, #0x1c8]   // r2 = static module RAM address for ARM9
    ldr     r0, =OSi_LtdMainParams
    ldr     r0, [r0]
