...
@22:            subs    r4, r4, #1              //      for ( i = 8; --i >= 0; ) {
                ...
                ldrb    r6, [r0], #1            //              *srcp++;
                swpb    r6, r6, [r1]            // r1:          *destp++; (Byte-writing countermeasure)
                add     r1, r1, #1
...
@24:            ldrb    r5, [r1, -r12]          //                  *destp++ = destp[-offset]
                swpb    r5, r5, [r1]            //    (Byte-writing countermeasure)
                add     r1, r1, #1
...
//* Although it can also expand in data TCM and main memory, it is slower than MI_UncompressLZ77().
//   
...
//With NITRO, cannot decompress directly into VRAM
//- If the compressed data size was not a multiple of four, adjust by padding with 0s as much as possible.
//  
//- Use 4 byte alignment for the source address.
//
...
@42:            ldrb    r3, [r0], #1            //          do {
                swpb    r3, r3, [r1]            //              *destp++ = *srcp++; (Byte-writing countermeasure)
                add     r1, r1, #1
...
@44:            swpb    r4, r5, [r1]            //          do { (Byte writing countermeasure)
...
//- Can be decompressed to the data TCM and VRAM
//- When decompressing to the main RAM, it is slower than MI_Uncomressed RL80
//- When decompressing to data TCM or VRAM, it is faster than UncompresedRL90.
//- If the compressed data size was not a multiple of four, adjust by padding with 0s as much as possible.
//  
//- Use 4 byte alignment for the source address.
//
...
//- Higher speed than MI_UncompressRL8(), MI_UncompressRL16()
//- If the compressed data size was not a multiple of four, adjust by padding with 0s as much as possible.
//  
//- Use 4 byte alignment for the source address.
//
...
@62                                             //          do {
                ...
                subs    r2, r2, #1              //              destCount--; Executes at this position in order to eliminate pipeline stall
...
@64                                             //          do {
                ...
                subs    r2, r2, #2              //              destCount -= 2; Executes at this position in order to eliminate pipeline stall
                add     r3, r3, r4              //              sum += tmp;
...
//- Can be decompressed to the data TCM and VRAM
//- Higher speed than MI_UnfilterDiff16()
//- If the compressed data size was not a multiple of four, adjust by padding with 0s as much as possible.
//  
//- Use 4 byte alignment for the source address.
//
...
@62                                             //          do {
                ldrh    r4, [r0, #2]!           //          tmp = *(u16*)srcp; srcp += 2;
                sub     r2, r2, #2              //              destCount -= 2; Executes at this position in order to eliminate pipeline stall
                add     r3, r3, r4              //              sum += tmp
...
//- Can be decompressed to the data TCM and VRAM
//  Faster than the MI_Uncompress8 function//---- 
//- If the compressed data size was not a multiple of four, adjust by padding with 0s as much as possible.
//  
//- Use 4 byte alignment for the source address.
//
...
//----------------------------------------------------------------------
//          8-bit expansion of the differential filter conversion 
//
//- Converts the differential filter, writing in 8 bit units.
//With NITRO, cannot decompress directly into VRAM
//- If the compressed data size was not a multiple of four, adjust by padding with 0s as much as possible.
//  
//- Use 4 byte alignment for the source address.
...
//    BOOL bitsize            Differential size (TRUE: 16bit, FALSE: 8bit)
//
//- Data header
//    u32 :4 :                 Bit size of unit
//        compType:4          Compression type( = 3)
...
//----------------------------------------------------------------------
//          16-bit expansion of the differential filter conversion 
//
//- Converts the differential filter, writing in 16 bit units.
//- Can be decompressed to the data TCM and VRAM
//- Higher speed than MI_FilterDiff8()
//- If the compressed data size was not a multiple of four, adjust by padding with 0s as much as possible.
//  
//- Use 4 byte alignment for the source address.
//
...
//----------------------------------------------------------------------
//          32-bit expansion of the differential filter conversion 
//
//- Converts the differential filter, writing in 32 bit units.
//- Higher speed than MI_FilterDiff16()
//- If the compressed data size was not a multiple of four, adjust by padding with 0s as much as possible.
//  
//- Use 4 byte alignment for the source address.
//
