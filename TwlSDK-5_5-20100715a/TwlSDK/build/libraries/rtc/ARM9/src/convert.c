...
// * Limited to the range of years that the RTC can take, 2000-2099.
static inline BOOL RTCi_IsLeapYear(u32 year)
{
    // A leap year is a year that is divisible by four and not divisible by 100, or a year that is divisible by 400
    return !((year & 0x03));
}
...
/*---------------------------------------------------------------------------*
  ...
  Returns:      The total number of days since January 1, 2000.
 *---------------------------------------------------------------------------*/
s32 RTC_ConvertDateToDay(const RTCDate *date)
{
    ...
    // Convert the month and day to the number of days
    dayNum = (s32)(date->day - 1);
    dayNum += sDayOfYear[date->month - 1];
    if ((date->month >= 3) && RTCi_IsLeapYear(date->year))
    {
        // In a leap year, increment days by one starting in March
        dayNum++;
    }
    ...
    // Because date->year is only from 0 up to 99, the leap year calculation is a simplified version.
...
/*---------------------------------------------------------------------------*
  ...
  Returns:      Total number of seconds since 0:00, January 1, 2000, local time.
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*
  ...
  Arguments:    date      - Pointer to save destination date data
                day       - The total number of days since January 1, 2000
...
/*---------------------------------------------------------------------------*
  ...
  Arguments:    date      - Pointer to save destination date data
                ...
                sec       - Total number of seconds since 0:00, January 1, 2000
