...
/*---------------------------------------------------------------------------*
  ...
  Description:  Reads the NTR binary FAT from the device.

  Arguments:    archive: The WFSTableFormat structure to initialize
                allocator: The allocator used to internally allocate memory
                device: The device storing the target FAT.
                origin_a: The device-internal offset for the ROM loading the source FAT.
                origin_b: The device-internal offset for the ROM that merges the overlay.
                   ...
                   MIDevice *device, u32 origin_a, u32 origin_b)
{
    ...
    /* Load the A header and record it in fat/fnt */
    {
        u8                      buf[0x60];
    ...
    /* Load the B header and record it in ov9/ov7 */
    {
        u8                      buf[0x60];
        const CARDRomHeader    *header = (const CARDRomHeader *)buf;
    ...
    if (allocator)
    {
        ...
        if (archive->buffer)
        {
            /* Load either A or B's fat/fnt/ov9/ov7. NOTE: origin has been adjusted. */
            {
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Parses the memory that read the NTR binary FAT image.

  Arguments:    archive: The WFSTableFormat structure to initialize
                                 The FAT image must be configured in advance in the members 'buffer' and 'length'.
...
void WFS_ParseTable(WFSTableFormat *archive)
{
    ...
    pos += sizeof(u32);
    /* Extract information about each region from the given buffer */
    for (i = 0; i < WFS_TABLE_REGION_MAX; ++i)
    {
