...
//// List of port numbers currently being used in data-sharing
...
/*---------------------------------------------------------------------------*
  ...
  Arguments:    dsInfo:   WMDataSharingInfo structure.
                sendData:        Send data that you want to share.
...
WMErrCode WM_StepDataSharing(WMDataSharingInfo *dsInfo, const u16 *sendData, WMDataSet *receiveData)
{
    ...
    if (aid == 0)
    {
        ...
        if (sendFlag)
        {
            ...
            WmDataSharingReceiveData(dsInfo, 0, (u16 *)sendData);       // Remove const because chained rewrite past here is bothersome.
    ...
    else
    {
        ...
        if (sendFlag)
        {
            ...
            int     res;
            // Use part of dsInfo->ds as a send buffer
...
static void WmDataSharingSetDataCallback(void *callback)
{
    ...
    // Checks if WM_EndDataSharing() has been called while DSInfo has been called in a invalid state.
    ...
    else
    {
        ...
        {
            // Resend because the transmission queue was full
            if (aid != 0)
            {
...
/*---------------------------------------------------------------------------*
  ...
  Description:  A child port receive callback.
        ...
        case WM_STATECODE_PORT_RECV:
            {
                ...
                if (length != dsInfo->dataSetLength)
                {
                    ...
                    {
                        // If incoming data is longer than the buffer, adjust it.
                ...
                {
                    // Larger than the aidBitmap expected by the child
...
void WmDataSharingSendDataSet(WMDataSharingInfo *dsInfo, BOOL delayed)
{
    ...
    enabled = OS_DisableInterrupts();
    // If receiving is finished for all the devices (The ds[].aidBitmap, from the receive buffer, carries out the role of a not-yet-received flag)
    //  
...
//        SDK_ASSERT( dsInfo->sendIndex == dsInfo->writeIndex);
        ...
        dsInfo->ds[oldWI].aidBitmap = dsInfo->aidBitmap;        // Enter the original value in aidBitmap before sending
        if (delayed == TRUE)
        {
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Gets a specific AID address during a session of receiving data in data sharing.
...
/*---------------------------------------------------------------------------*
  ...
  Arguments:    dsInfo:   WMDataSharingInfo structure.
                aidBitmap:   Other communication peer(s) included in the data.
                receiveBuf:   Receive buffer
                aid:           AID.
                                Confirm aidBitmap & (1<<aid) before calling this.
...
u16    *WmGetSharedDataAddress(WMDataSharingInfo *dsInfo, u32 aidBitmap, u16 *receiveBuf, u32 aid)
{
    ...
    // Count the '1's among the aid bits in the aidBitmap, from the bottom
    mask = (0x0001U << aid) - 1U;
    aidBitmap &= mask;
