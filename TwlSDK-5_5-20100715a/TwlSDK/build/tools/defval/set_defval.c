...
/*---------------------------------------------------------------------------*
  ...
  Description:  Adds a new value to the defined value list.
  
  Arguments:    name        Variable name
                value   Variable's value
  
                If name  is NULL or "", then do nothing and end (Return FALSE)
                If value is NULL, it is treated as being defined as "".
  
  Returns:      TRUE   Added a new one
...
static BOOL add_dvalue(const char *name, const char *value)
{
    ...
    //
    // Handling when NULL
    //   If name is NULL or "", then do nothing and end (Return TRUE)
    //   If value is NULL, it is treated as being defined as "".
    //
...
BOOL add_dvalue_by_equality(const char *equality)
{
    ...
    name = clip_whitespace(name);      // Delete white space before and after.
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Adds equation inside the file to the definition value list.
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Delete white space before and after character string.
  
  Arguments:    str    Beginning of the string to process
  
                The contents of this str will be overwritten, so it does not work with a const area
