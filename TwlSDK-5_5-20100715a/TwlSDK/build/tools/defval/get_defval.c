...
/*---------------------------------------------------------------------------*
  ...
  Description:  Gets the value of the definition name.
                Search the definition value list; If not there, search the environment variables
                If still not there, return NULL.
  
  Arguments:    name        Variable name
  
  Returns:      Pointer to the value string (No need to free)
                NULL: No corresponding variables
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Duplicates and returns the definition values after converting them using variable modifiers (:r, :t, :e, :h).
...
static char *modify_dvalue(const char *value, char modifier)
{
    ...
    switch (modifier)
    {
    case 'h':                         // Directory name with final /, \ included
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Dupicates and returns value from the definition value list, after appying conversion based on modifier (:r,:t,:e,:h).
...
BOOL puts_modified_dvalue(FILE * fp, const char *str)
{
    ...
    {
        //
        // Search for $(XXX); If found, output corresponding value.
        //
        if ('$' == *str)
        {
            if ('$' == *(str + 1))
        ...
        //
        // If not $(XXX), output one character at a time
        //
        if (EOF == fputc(*str, fp))
        {
            return FALSE;
