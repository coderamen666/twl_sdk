...
/*---------------------------------------------------------------------------*
  ...
  Description:  Version of fopen with stdin/stdout taken into account.
                If '-' is specified and also access mode is 'r', set to stdin.
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Version of fclose with stdin/stdout taken into account.
                If NULL, stdin, stdout or stderr, does not close
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Reads the file one line at a time.
                Unlike the normal fgets, uses dynamic control of the buffer size to precisely read just one line.
  ...
  Arguments:    *buffer  (Pointer to) the pointer to the file buffer
      ...
      Passing 'size' as an argument reuses 'buffer' so that alloc/free processing is reduced.
       
      It is expected that the first call will be to set *buffer to NULL.
...
char   *Fgets(char **pbuffer, int *pbuffer_size, FILE * fp)
{
    ...
    while (NULL != fgets(buffer + buffer_gets, buffer_size - buffer_gets, fp))
    {
        ...
        //    In loading done by fgets, the end of a line is determined when:
        ...
        //
        //  If the loaded line does not reach the end of the line, double the buffer size, and continue reading.
        //   
        //
        else
        {
