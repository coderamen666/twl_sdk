...
/*  */
enum {
  PT_NULL,    /* An unused entry; the meaning of other member values is undefined */
  PT_LOAD,    /* Segment that will be loaded at runtime */
  PT_DYNAMIC, /* Segment that stores a dynamic structure array */
  PT_INTERP,  /* Segment that stores the path of the interpreter used to parse the file */
  PT_NOTE,    /* Segment that stores information not used to parse the file */
  PT_SHLIB,   /* reserved */
  PT_PHDR,    /* Program header table (exists only when part of the program's memory image is used) */
  PT_TLS,     /* Template for the thread-local storage segment */

  PT_LOOS = 0x60000000,
  /* Region reserved for OS-specific semantics */
  PT_HIOS = 0x6fffffff,

  PT_LOPROC = 0x70000000,
  /* Region reserved for processor-specific semantics */
...
/*---------------------------------------------------------
 ...
 --------------------------------------------------------*/
typedef struct {
  Elf32_Word    sh_name;        /*Index to an entry in the table section with section header strings*/
...
#define STB_MW_SPECIFIC 14    /*This shows up in NITRO applications (OS_TPrintf is one example)*/
