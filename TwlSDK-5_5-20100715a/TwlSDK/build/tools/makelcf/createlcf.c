/*---------------------------------------------------------------------------*
  ...
  Support for FOREACH.AUTOLOADS=XXX notation
  ...
  Support for .itcm.bss, .dtcm.bss, and .wram.bss
...
static int ForeachObjectsForce(tObject ** ptrObject, tObject * start, tForeachStatus* fep, int n)
{
    tTokenBuffer *token;
    char   *section;
    char    filter;
...
{
    return ForeachObjectsForce(&StaticForce, Static.forces.head, &Foreach.static_force, n);
}

// If Force First/Last
static void PutIfStaticForceFirst(const char *str)
{
    PutIfObject(Foreach.static_force.isFirst, str);
}

static void PutIfStaticForceLast(const char *str)
{
    PutIfObject(Foreach.static_force.isLast, str);
}
...
static int ForeachAutoloadForces(int n)
{
    if (Autoload)
    {
        return ForeachObjects(&AutoloadForce, Autoload->forces.head, &Foreach.autoload_force, n);
    }
    else
    {
        lcf_error("No <FOREACH.AUTOLOADS> Ignored.");
...
// If Force First/Last
static void PutIfAutoloadForceFirst(const char *str)
{
    PutIfObject(Foreach.autoload_force.isFirst, str);
}

static void PutIfAutoloadForceLast(const char *str)
{
    PutIfObject(Foreach.autoload_force.isLast, str);
}
...
static int ForeachOverlayForces(int n)
{
    if (Overlay)
    {
        return ForeachObjects(&OverlayForce, Overlay->forces.head, &Foreach.overlay_force, n);
    }
    else
    {
        lcf_error("No <FOREACH.OVERLAYS> Ignored.");
...
// If Force First/Last
static void PutIfOverlayForceFirst(const char *str)
{
    PutIfObject(Foreach.overlay_force.isFirst, str);
}

static void PutIfOverlayForceLast(const char *str)
{
    PutIfObject(Foreach.overlay_force.isLast, str);
}
...
static int PutToken(int n)
{
    ...
    case tIF_STATIC_FORCE_FIRST:
        PutIfStaticForceFirst(tokenBuffer[n].string);
        break;
        
    case tIF_STATIC_FORCE_LAST:
        PutIfStaticForceLast(tokenBuffer[n].string);
        break;
    ...
    case tIF_AUTOLOAD_FORCE_FIRST:
        PutIfAutoloadForceFirst(tokenBuffer[n].string);
        break;
        
    case tIF_AUTOLOAD_FORCE_LAST:
        PutIfAutoloadForceLast(tokenBuffer[n].string);
        break;
    ...
    case tIF_OVERLAY_FORCE_FIRST:
        PutIfOverlayForceFirst(tokenBuffer[n].string);
        break;
        
    case tIF_OVERLAY_FORCE_LAST:
        PutIfOverlayForceLast(tokenBuffer[n].string);
        break;
