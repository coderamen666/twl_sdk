 ...
 *----------------------------------------------------------------------*/
#define HW_SYSM_VER_INFO_CONTENT_ID					(HW_MAIN_MEM + 0x00ffd7b0 ) // 8-byte, NULL-terminated contentID
 ...
 *----------------------------------------------------------------------*/
#define HW_SD_NAND_CONTEXT_BUF					(HW_MAIN_MEM + 0x00ffd7bc )	// NAND context data for the SD driver taken from the launcher
 ...
 *----------------------------------------------------------------------*/
#define HW_TWL_FS_MOUNT_INFO_BUF                (HW_MAIN_MEM + 0x00ffdc00 ) // Mount information (A size of 0x3c0 is allocated. You can specify up to 11 mount points. 11*84+1=0x39d)
#define HW_TWL_FS_BOOT_SRL_PATH_BUF             (HW_MAIN_MEM + 0x00ffdfc0 ) // Path to the SRL file for startup
...
#define HW_MAIN_MEM_SYSTEM_SIZE         (HW_MAIN_MEM_SYSTEM_END - HW_MAIN_MEM_SYSTEM)   // 1024 bytes
...
#define HW_HW_SECURE_INFO				(HW_MAIN_MEM + 0x00fffd68)      // Secure system information (END-0x298)
#define HW_HW_SECURE_INFO_END			(HW_MAIN_MEM + 0x00fffd80)      //                           (END-0x280)

#define HW_BIOS_EXCP_STACK_MAIN         (HW_MAIN_MEM + 0x00fffd80)      // MAINP Debugger monitor exception handler(END-0x280)
#define HW_BIOS_EXCP_STACK_MAIN_END     (HW_MAIN_MEM + 0x00fffd9c)      //                                 (END-0x264)
#define HW_EXCP_VECTOR_MAIN             (HW_MAIN_MEM + 0x00fffd9c)      // HW_EXCP_VECTOR_BUF for MAINP      (END-0x264)

#define HW_ARENA_INFO_BUF               (HW_MAIN_MEM + 0x00fffda0)      // Arena data structure (27F_FDA0 to 27F_FDE7) (END-0x260)
...
#define HW_TWL_RED_LAUNCHER_VER			(HW_MAIN_MEM + 0x00fffdfb)      // RED launcher version
#define HW_PRELOAD_PARAMETER_ADDR       (HW_MAIN_MEM + 0x00fffdfc)      // preload parameter address

#define HW_ROM_HEADER_BUF               (HW_MAIN_MEM + 0x00fffe00)      // ROM-internal registration area data buffer (END-0x200)
...
#define HW_MIC_LAST_ADDRESS             (HW_MAIN_MEM + 0x00ffff90)      // MIC new sampling data storage address
#define HW_MIC_SAMPLING_DATA            (HW_MAIN_MEM + 0x00ffff94)      // MIC individual sampling results

#define HW_WM_CALLBACK_CONTROL          (HW_MAIN_MEM + 0x00ffff96)      // Parameter to synchronize WM callback
#define HW_WM_RSSI_POOL                 (HW_MAIN_MEM + 0x00ffff98)      // Random number source depends on WM received signal intensity
...
#define HW_COMPONENT_PARAM              (HW_MAIN_MEM + 0x00ffff9c)      // Parameter for component synchronization

#define HW_THREADINFO_MAIN              (HW_MAIN_MEM + 0x00ffffa0)      // ThreadInfo for Main processor
#define HW_THREADINFO_SUB               (HW_MAIN_MEM + 0x00ffffa4)      // ThreadInfo for subprocessor
...
#define HW_LOCK_ID_FLAG_SUB             (HW_MAIN_MEM + 0x00ffffb8)      // lockID flag for subprocessor

// SpinLock Mutex
#define HW_VRAM_C_LOCK_BUF              (HW_MAIN_MEM + 0x00ffffc0)      // VRAM-C lock buffer (END-0x40)
#define HW_VRAM_D_LOCK_BUF              (HW_MAIN_MEM + 0x00ffffc8)      // VRAM-D lock buffer (END-0x38)
#define HW_WRAM_BLOCK0_LOCK_BUF         (HW_MAIN_MEM + 0x00ffffd0)      // CPU internal work RAM - Block0 - lock buffer (END-0x30)
#define HW_WRAM_BLOCK1_LOCK_BUF         (HW_MAIN_MEM + 0x00ffffd8)      // CPU internal work RAM - Block1 - lock buffer (END-0x28)
#define HW_CARD_LOCK_BUF                (HW_MAIN_MEM + 0x00ffffe0)      // Game Card - lock buffer (END-0x20)
#define HW_CTRDG_LOCK_BUF               (HW_MAIN_MEM + 0x00ffffe8)      // DS Pak - lock buffer (END-0x18)
#define HW_INIT_LOCK_BUF                (HW_MAIN_MEM + 0x00fffff0)      // Initialization lock buffer  (END-0x10)

#define HW_MMEMCHECKER_MAIN             (HW_MAIN_MEM + 0x00fffff8)      // MainMemory Size Checker for Main processor (END-8)
#define HW_MMEMCHECKER_SUB              (HW_MAIN_MEM + 0x00fffffa)      // MainMemory Size Checker for Sub processor  (END-6)

#define HW_CHIPTYPE_FLAG				(HW_MAIN_MEM + 0x00fffffc) 		// chiptype flag (END-4)

#define HW_CMD_AREA                     (HW_MAIN_MEM + 0x00fffffe)      // Main memory command issue area (Prohibited use area) (END-2)
...
/*----------------------------------------------------------------------*
    Shared memory space for checking the debugger state
  [TODO] This region will no longer be reserved by the system when starting in TWL mode, so someone needs to revise the debugger library.
