...
/*---------------------------------------------------------------------------*
  ...
  Arguments:    context:          The WFSServerContext structure.
                ...
                overlay:          The offset in the device to the NTR binary containing the overlay to be merged.
                                  (This has the same value as "fatbase" if multiple ROM file tables are not merged)
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Sets the clients that synchronously access the server.
                This function synchronizes responses to clients that are clearly guaranteed to access the same files in precisely the same order, thereby achieving efficient transmission rates that utilize the unique characteristics of the WBT library.
                Note, however, that responses will grow out-of-sync and cause deadlocks if synchronization starts at logically insecure times.
  ...
  Arguments:    context:          The WFSServerContext structure.
                bitmap:           The AID bitmap for the client to be synchronized.
                                 Synchronization will not occur (default behavior) when 0 is specified.
                                 The lowest bit is always ignored.
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Registers the specified ROM file with the WFS library for distribution, and automatically starts an internal ROM access thread.
  ...
  Arguments:    context:          The WFSServerContext structure.
                ...
                sharedFS:         TRUE if the file system is to be shared by the parent and child.
                                 In this case, only the overlay held by 'file' is extracted and appended to the file system of the parent device itself to make a combined file system to distribute.
                                 This argument is ignored if "file" is set to NULL, indicating a clone boot (this will always be interpreted as TRUE).
