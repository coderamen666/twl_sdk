...
typedef struct OSOwnerInfo
{
    ...
    // Nickname (Max. 10 characters of Unicode (UTF16), no terminating code)
    u16     nickNameLength;            // Number of characters in nickname (0 to 10)
    u16     comment[OS_OWNERINFO_COMMENT_MAX + 1];
    // Comment (Max. 26 characters of Unicode (UTF16), no terminating code)
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Gets the offset (in units of seconds) which fluctuates in response to the new value every time the owner changes the RTC setting.
                ...
                If the RTC is pushed forward into the future relative to the RTC time prior to being reset, the pushed-forward time is added.
                 If the RTC is pushed back into the past, the pushed-back time is subtracted.
...
/*---------------------------------------------------------------------------*
  ...
  Returns:      Pointer to the GXRgb type constant array that stores corresponding color values.
 *---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Gets the "Favorite Color" that corresponds to the specified number.
