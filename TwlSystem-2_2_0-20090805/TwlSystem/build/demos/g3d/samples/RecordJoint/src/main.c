...
//---------------------------------------------------------------------------
//
// This sample pre-calculates a joint's calculation results.
//
// Buffering a joint's calculation results can save CPU calculation costs when a joint is unchanging.
// 
//
// Sample Operation
// A visibility animation is played with looped repeat.
// Press the A Button to switch between using the buffered joint calculation result or to recalculate for each rendering.
// 
//
// HOWTO:
// 1: Secure the buffer region with NNS_G3dAllocRecBufferJnt.
// 2: Attach the buffer to the rendering object with the NNS_G3dRenderObjSetJntAnmBuffer function.
//    
...
// 4: In addition, no data is sent to the geometry engine when NNS_G3dDraw is run with the NNS_G3D_RENDEROBJ_FLAG_NOGECMD flag set.
//    
...
//    
// 7: Starting with the next NNS_G3dDraw, instead of calculating joint data, reference the buffer data and render.
// 8: You can release the buffer with NNS_G3dRenderObjReleaseJntAnmBuffer.
//
//---------------------------------------------------------------------------
...
NitroMain(void)
{
    ...
    //
    // Load the visibility animation and try to associate it.
    //
    {
        ...
        {
            //
            // Allocate the joint calculation results buffer and attach it to the rendering object.
            //
            pBufJntAnmResult = NNS_G3dAllocRecBufferJnt(&allocator, pModel);
            NNS_G3dRenderObjSetJntAnmBuffer(&object, pBufJntAnmResult);

            //
            // Record the joint calculation results in the buffer.
            //
            NNS_G3dRenderObjSetFlag(&object, NNS_G3D_RENDEROBJ_FLAG_NOGECMD); // do not send to geometry engine
            NNS_G3dRenderObjSetFlag(&object, NNS_G3D_RENDEROBJ_FLAG_RECORD);  // record data in pBufJntAnmResult

            NNS_G3dDraw(&object);

            NNS_G3dRenderObjResetFlag(&object, NNS_G3D_RENDEROBJ_FLAG_NOGECMD); // send to geometry engine
            NNS_G3dRenderObjResetFlag(&object, NNS_G3D_RENDEROBJ_FLAG_RECORD);  // use the pBufJntAnmResult data
        }
    ...
    for(;;)
    {
        ...
        {
            //
            // Switch by pressing the A Button.
            //
            if (useRecord)
            {
                useRecord = FALSE;
