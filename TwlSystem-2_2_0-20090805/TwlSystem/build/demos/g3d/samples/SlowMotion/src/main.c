...
//---------------------------------------------------------------------------
//
// This sample plays in slow motion.
//
// 
// When the ICA file's <node_anm_info>::interpolation attribute is set to linear, the joint animation is played even if you have specified a non-decimal frame number, and the frame will be interpolated in context.
// When looping animation,
// be sure to set the <node_anm_info>::interp_end_to_start attribute to on.
// When interpolating non-decimal frames, take into consideration that the speed becomes slower than that without the interpolation.
// 
//
// Sample Operation
// A person is running in slow motion.
// Press the A Button to switch use of interpolated playback on and off.
//
// HOWTO:
// 1. Check that the <node_anm_info>::interpolation attribute of the ICA file is set to linear.
//    
// 2. Check that the <node_anm_info>::interp_end_to_start attribute of the ICA file is on.
//    
// 3. Animation is played back without the non-decimal frame being discarded as long as it is handled as in the JointAnm sample.
//    
// 
//---------------------------------------------------------------------------
...
NitroMain(void)
{
    ...
    for(;;)
    {
        ...
        //
        // When there are non-decimal frames, determine whether to perform interpolation.
        // Direct manipulation of the binary resource flag corresponding to the ICA file's <node_anm_info>::interpolation attribute will enable switching even at runtime.
        // 
        //
        if (doInterp)
        {
            // When you want to switch the non-decimal frame playback on
            ((NNSG3dResJntAnm*)(pAnmObj->resAnm))->flag |= NNS_G3D_JNTANM_OPTION_INTERPOLATION;
        }
        else
        {
            // When you want to switch the non-decimal frame playback off
            ((NNSG3dResJntAnm*)(pAnmObj->resAnm))->flag &= ~NNS_G3D_JNTANM_OPTION_INTERPOLATION;
        }
