...
//---------------------------------------------------------------------------
//
// This sample accomplishes the same behavior as the callback2 sample without using a callback.
//
// Using the -s option with g3dcvtr allows calculated matrices to always be stored on the matrix stack.
// Doing so allows you to get the necessary joint matrix by restoring the matrix stored on the stack after rendering to the current matrix.
// 
// 
//
// Sample Operation
// A person is running while holding a walking robot in their left hand. The operations for this sample are the same as the callback2 sample.
//
// HOWTO:
// 1: Convert the .imd file, using the -s option.
// 2: Render a running person with NNS_G3dDraw.
// 3: Specify the needed joint's jntID in the NNS_G3dGetResultMtx function to restore the matrix to the current matrix.
//    
// 4: When the robot is rendered with the NNS_G3dDraw function, the running person's left hand will be holding the robot (in other words, the robot will be stuck to the person's hand).
//    
//
//---------------------------------------------------------------------------
...
NitroMain(void)
{
    ...
    for(;;)
    {
 ...
 
        {
            ...
            //
            // Since the -s option is used when converting the person's IMD file, all of the matrices calculated at rendering are stored on the matrix stack.
            // 
            // (Therefore, you can only convert a file with 30 nodes or fewer.)
            // Compared to the callback2 sample, there's no need to get a matrix during rendering, so there's no need to use callbacks.
            // 
            //
            int jntID;
            BOOL result;

            // Get joint ID of left hand.
            NNS_G3D_GET_JNTID(object.resMdl, &jntID, "wrist_l_end");

            //
            // Bring the matrix corresponding to the jntID node of the rendering object to the current matrix.
            // You can get the location coordinate matrix by specifying the pointer to MtxFx43 in the second argument, but you can get the direction vector matrix by specifying the pointer to MtxFx33 in the third argument.
            // 
            //
            result = NNS_G3dGetResultMtx(&object, NULL, NULL, (u32)jntID);

            // FALSE is returned when there is no matrix corresponding to the jntID node, so check just to be sure.
            SDK_ASSERT(result);

            //
            // Since posScale scaling only occurs right before rendering, you must perform explicit scaling.
            // 
            //
