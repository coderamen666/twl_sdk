...
//---------------------------------------------------------------------------
// This sample displays environment-mapped objects.
//
// When the IMD file's <material> element's tex_gen_mode is set to nrm, texture mapping will use an environment map.
// 
//
// Sample Operation
...
rotEnvMap(NNSG3dRS* rs)
{
    ...
    // The matrix mode must remain GX_MTXMODE_TEXTURE even after the callback function is complete.
    // 
    //
    static MtxFx33 mRot = {
        FX32_ONE, 0, 0,
...
NitroMain(void)
{
    ...
    //
    // When customizing effect_mtx settings,
    // insert callback processing using timing B for NNS_G3D_SBC_ENVMAP.
    // 
    //
